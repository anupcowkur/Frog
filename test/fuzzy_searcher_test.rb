...
class TestFuzzySearcher < Minitest::Test
  ...
  def test_that_it_errors_out_on_not_finding_search_term
    ...
    FuzzySearcher.new(mock_error_handler).search(@links, "goooo")
    mock_error_handler.verify
  end

  def test_that_it_displays_correct_options_on_finding_search_term
    fake_io("1") do
      FuzzySearcher.new(ErrorHandler.new).search(@links, "running")
  ...
  def test_that_it_returns_correct_url_when_user_selects_an_index
    fake_io("1") do
      assert_equal "https://developer.android.com/reference/android/app/ActivityManager.RunningAppProcessInfo.html", FuzzySearcher.new(ErrorHandler.new).search(@links, "running")
    end
  end
  ...
  def test_that_it_errors_out_when_user_selects_an_incorrect_index
    ...
    fake_io("99") do
      FuzzySearcher.new(mock_error_handler).search(@links, "running")
    end
    mock_error_handler.verify
  end
end
